...
static void init_kernel(struct krnl_t *k)
{
    k->ready_queue     = NULL;
    k->running_list    = NULL;

#ifdef MLQ_SCHED
    k->mlq_ready_queue = NULL;
#endif

#ifdef MM_PAGING
    /* mấy field này loader sẽ gán sau */
    k->mm              = NULL;
    k->mram            = NULL;
    k->mswp            = NULL;
    k->active_mswp     = NULL;
...
/*CPU thread: CPU sẽ “đọc lệnh + chạy” thôi.*/
...
static void * cpu_routine(void * args)
{
    ...
    while (1) {
        ...
        if (proc == NULL) {
            /* Lấy process mới nếu CPU đang rảnh */
            /* No process is running, then we load new process from
             * ready queue */
            proc = get_proc();
            if (proc == NULL) {
                next_slot(timer_id);
                /* First load failed. skip dummy load */
                continue;
            }
            ...
            /* Hết time slot thì trả lại queue */
...
static void * ld_routine(void * args)
{
    ...
    while (i < num_processes) {
        ...
        /* Liên kết PCB với kernel để dùng syscall*/
        proc->krnl = &os;
...
/* khởi tạo, spawn thread và đợi CPU chạy xong.*/
int main(int argc, char * argv[])
{
    ...
    /* Init kernel để tránh rác – bước này nên có sớm */
    init_kernel(&os);
